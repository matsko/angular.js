  ...
  .factory('$animateViewPort', ['$$animateSequence', '$animate', '$$animateChildren', '$$compileInlineTemplate', '$$animateKeep',
    ...
    return function(element, scope) {
      ...
      return {
        ...
        enter : function(parent, after, options) {
          ...
          if (steps.length) {
            var startSequence = enterAnimation(element, steps, animationCtrl.enter.keeps);
        ...
        leave : function(options) {
          ...
          if (steps.length) {
            var startSequence = leaveAnimation(element, steps, animationCtrl.leave.keeps);
    ...
    function enterAnimation(container, steps, keeps) {
      ...
      angular.forEach(steps, function(step) {
        if (!step.keep) {
          ...
          angular.forEach(elements, function(element) {
            if (!$$animateKeep.proxied(element)) {
              angular.element(element).addClass(NG_ANIMATE_QUEUED_CLASS_NAME);
            }
          });
        } else {
          var element = container[0].querySelector(step.selector);
          if (element && $$animateKeep.query(step.selector)) {
            $$animateKeep.proxy(element, true);
          }
        }
      });
      return $$animateSequence(container, steps, function(element, step, index) {
        ...
        var promise;
        if (step.keep) {
          promise = $animate.animate(element, step.from, step.to, 'ng-enter-keep');
          promise.then(function() {
            var target = container[0].querySelector(step.selector);
            $$animateKeep.attach(step.selector, target);
          });
        } else {
          promise = $animate.animate(element, step.from, step.to, 'ng-enter', {
  ...
  .factory('$$animateKeep', ['$document', function($document) {
    ...
    return self = {
      calculate : function(element) {
        var data = (element.length ? element[0] : element).getBoundingClientRect();
        var coords = {};
      ...
      proxy : function(element, bool) {
        angular.element(element)[bool ? 'addClass' : 'removeClass']('ng-animate-proxied');
      },
      proxied : function(element) {
        return angular.element(element).hasClass('ng-animate-proxied');
      },
      attach : function(selector, target) {
        var record = lookup[selector];
        if (record) {
          record.element.remove();
          delete lookup[selector];
        }
        self.proxy(target, false);
      },
